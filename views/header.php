<!DOCTYPE html>
<html ng-app="shop">
	<head>

		<meta charset="utf-8">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<title>Borussia</title>
		<link rel="icon" type="image/png" href="public/images/icons/favicon.ico"/>
		<link rel="stylesheet" href="https://stackpath.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css" integrity="sha384-Vkoo8x4CGsO3+Hhxv8T/Q5PaXtkKtu6ug5TOeNV6gBiFeWPGFN9MuhOf23Q9Ifjh" crossorigin="anonymous">
		<link rel="stylesheet" href="lib/owl.carousel/owl-carousel/owl.carousel.css">
		<link rel="stylesheet" href="public/fonts/font-awesome/css/font-awesome.min.css">
		<link rel="stylesheet" href="public/css/stylesheet.css">
		<link rel="stylesheet" href="public/css/stylesheet-mobile.css">


	</head>
	<body>
		
	<header>
			
	
		<div id="menu-mobile-mask" class="container"></div>

		<div id="menu-mobile" class="container">
			
			<ul class="list-unstyled">

				<li><a href="http://borussia.ext2.com.br.br/">Início</a></li>
				
				<li>

					<div class="dropdown">
						<a class="btn dropdown-toggle" href="https://ext2.com.br" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
							Portfólio
						</a>

						<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
							
							<a class="dropdown-item" href="https://ext2.com.br">Portfólio</a>
							<a class="dropdown-item" href="https://wedding.ext2.com.br">Wedding</a>
							<a class="dropdown-item" href="https://plugin.ext2.com.br/formulario-de-contato">Plugin</a>
							
							<a class="dropdown-item" href="https://e-book.ext2.com.br">E-book Store</a>
							<a class="dropdown-item" href="https://linhaprojetos.ext2.com.br">Linha Projetos</a>
							<a class="dropdown-item" href="https://landing.ext2.com.br">Landing</a>
							<a class="dropdown-item" href="https://code.ext2.com.br">Code Sample</a>
						</div>
					</div>
				
				</li>
				
			</ul>

			<div class="bar-close">
				<button type="button" class="btn btn-close"><i class="fa fa-close"></i></button>
			</div>

		</div>
			
		<div class="container">
			<img id="logotipo" src="public/images/borussia-logo.png" style="width:128px; height:146px;" alt="Logotipo">
		</div>

		<div id="row-hr-1" class="container">
			
			<button id="btn-bars" type="button"><i class="fa fa-bars"></i></button>

		</div>






		<div class="container">
				
			<div class="row pull-right">
					
				<nav id="menu" class="col-12">
					<ul>
						<li><a href="https://borussia.ext2.com.br.br/">Início</a></li>
						
						<li>


							<div class="dropdown">
								<a class="btn dropdown-toggle" href="https://ext2.com.br.br" role="button" id="dropdownMenuLink" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
									Portfólio
								</a>

								<div class="dropdown-menu" aria-labelledby="dropdownMenuLink">
									
									<a class="dropdown-item" href="https://ext2.com.br">Portfólio</a>
									<a class="dropdown-item" href="https://wedding.ext2.com.br">Wedding</a>
									<a class="dropdown-item" href="https://plugin.ext2.com.br/formulario-de-contato">Plugin</a>
									
									<a class="dropdown-item" href="https://e-book.ext2.com.br">E-book Store</a>
									<a class="dropdown-item" href="https://linhaprojetos.ext2.com.br">Linha Projetos</a>
									<a class="dropdown-item" href="https://landing.ext2.com.br">Landing</a>
									<a class="dropdown-item" href="https://code.ext2.com.br">Code Sample</a>
								</div>
							</div>


						</li>
					</ul>
				</nav>

			</div>

		</div>			

	</header>

